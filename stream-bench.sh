...
REDIS_VERSION=${REDIS_VERSION:-"3.2.11"}
...
run() {
  ...
  then
    ...
    cd ..
  elif [ "START_NEW_REDIS" = "$OPERATION" ];
  then
    start_if_needed redis-server Redis 1 "$REDIS_DIR/src/redis-server"
  ...
  then
    ...
    cd ..
  elif [ "STOP_NEW_LOAD" = "$OPERATION" ];
  then
    cd data
    $LEIN run -g --configPath ../$CONF_FILE || true
    cd ..
  elif [ "START_NEW_LOAD" = "$OPERATION" ];
  then
    cd gendata
    scala -classpath target/spark-benchmarks-0.1.0.jar spark.benchmark.KafkaAdvertisingProducer -t $TEST_TIME -l $LOAD &
    cd ..
