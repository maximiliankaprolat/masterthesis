...
object KafkaRedisAdvertisingStream {
  def main(args: Array[String]) {
	...
	// Word Count um Parameter zu verwenden - https://spark.apache.org/examples.html
	val words = kafkaRawData.flatMap(line => line.split(";")).map(word => (word, 1)).reduceByKey(_+_)
